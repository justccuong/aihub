...
export const datasourceGroupsQuerySchema = z.object({
...
export const datasourceGroupsParams = {
    page: createParser({
        parse: (value) => datasourceGroupsQuerySchema.shape.page.parse(value),
        serialize: (value) => String(value),
    }).withDefault(PAGINATION.DEFAULT_PAGE).withOptions({ clearOnDefault: true }),

    pageSize: createParser({
        parse: (value) => datasourceGroupsQuerySchema.shape.pageSize.parse(value),
        serialize: (value) => String(value),
    }).withDefault(PAGINATION.DEFAULT_PAGE_SIZE).withOptions({ clearOnDefault: true }),

    search: createParser({
        parse: (value) => datasourceGroupsQuerySchema.shape.search.parse(value),
        serialize: (value) => value,
    }).withDefault("").withOptions({ clearOnDefault: true }),

    sortBy: createParser({
        parse: (value) => datasourceGroupsQuerySchema.shape.sortBy.parse(value),
        serialize: (value) => value,
    }).withDefault('createdAt' as const).withOptions({ clearOnDefault: true }),

    sortOrder: createParser({
        parse: (value) => datasourceGroupsQuerySchema.shape.sortOrder.parse(value),
        serialize: (value) => value,
